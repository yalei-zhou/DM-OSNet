...
#             try:
#                 outputs1 = model[0](imgs, return_featuremaps=True)
#                 outputs2 = model[1](imgs)
...
#                     )
#                 )
#             outputslist = [outputs1,outputs2]

#             overlapped1 = None
#             overlapped2 = None
#             for zz,outputs in enumerate(outputslist):
...
#                     if zz == 0:
#                         overlapped1 = overlapped
#                     else:
#                         overlapped2 = overlapped
...
#                 grid_img[:,
#                          width + GRID_SPACING:2*width + GRID_SPACING, :] = overlapped1
#                 grid_img[:, 2*width + 2*GRID_SPACING:, :] = overlapped2
    ...
    for target in list(test_loader.keys()):
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            for j in range(outputs.size(0)):
                ...
                # save images in a single figure (add white spacing between images)
                # from le ft to right: original image, activation map, overlapped image
                grid_img = 255 * np.ones(
                    (height, 3*width + 2*GRID_SPACING, 3), dtype=np.uint8
                )
                grid_img[:, :width, :] = img_np[:, :, ::-1]
...
def main():
    ...
    args = Namespace(dataset='market1501', height=256, model='osnet_x1_0', root='/home/zyl/fast-reid/datasets/', save_dir='log/osnettranconv', weights='/home/zyl/fast-reid/weights/osnet_x1_0_market_256x128_amsgrad_ep150_stp60_lr0.0015_b64_fb10_softmax_labelsmooth_flip.pth', width=128)
    print(args)
    ...
    model2 = build_osnet_backbone()
    Checkpointer(model2).load("/home/zyl/fast-reid/weights/model_best_zl_osnet100.pth")
    # Checkpointer(model2)
    # Checkpointer.load(path="/home/zyl/fast-reid/weights/model_best_zl_osnet100.pth")
    if use_gpu:
        model1 = model1.cuda()
        model2 = model2.cuda()
    if args.weights and check_isfile(args.weights):
        load_pretrained_weights(model1, args.weights)

    # visactmap(
    #     [model1,model2], test_loader, args.save_dir, args.width, args.height, use_gpu
    # )
    visactmap(
        model2, test_loader, args.save_dir, args.width, args.height, use_gpu
    )
