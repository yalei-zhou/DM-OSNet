...
class Block(nn.Module):
    def __init__(self, dim, head_dim, grid_size=1, ds_ratio=1, expansion=4,
        ...
        super().__init__()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()#DropPath/drop_path 是一种正则化手段，其效果是将深度学习模型中的多分支结构随机”删除“
...
class TransCNN(nn.Module):
    def __init__(self, img_size=256, in_chans=3, num_classes=1000, dims=[64, 128, 256, 512],
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]#返回一个1维张量，包含在区间start和end上均匀间隔的step个点。
