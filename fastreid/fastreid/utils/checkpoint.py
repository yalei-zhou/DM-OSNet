...
class Checkpointer(object):
    ...
    def _load_model(self, checkpoint: Any):
        ...
        """
        checkpoint_state_dict = checkpoint.pop("model")#原定义
        
        # checkpoint_state_dict = checkpoint.pop("state_dict")
        # checkpoint_state_dict = checkpoint#deepreid提供的训练好的模型
        ...
        # _add_prefix(checkpoint_state_dict, "backbone.")
        _strip_prefix_if_present(checkpoint_state_dict, "backbone.")


        # work around https://github.com/pytorch/pytorch/issues/24139
        model_state_dict = self.model.state_dict()
        # _strip_prefix_if_present(model_state_dict,"backbone.")
        incorrect_shapes = []
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
        ...
        for k, shape_checkpoint, shape_model in incompatible.incorrect_shapes:
            ...
            print("Skip loading parameter '{}' to the model due to incompatible "
                "shapes: {} in the checkpoint but {} in the "
                "model! You might want to double check if this is expected.".format(
                    k, shape_checkpoint, shape_model))
...
def _add_prefix(state_dict: Dict[str, Any], prefix: str) -> None:
    """
    ...
    for key in keys:
        if  key.startswith("head."):#增加
            continue
        # newkey = key[len(prefix):]#原
        newkey = prefix+key
